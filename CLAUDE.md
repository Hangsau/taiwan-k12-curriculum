...
- **最後更新**：2026-06-25 下午（§19 P5 開放資料釋出 v0.5.0 — CITATION.cff + README 強化 + docs/release-notes.md + ROADMAP Phase 5 ✅）
- **當前 Phase**：P1 ✅ + P1.5 ✅ + P2 教材 ✅ + P2-B 考卷 ✅ + P2-B-2 .md ✅ + P3 知識圖譜 ✅ + P4 遊戲端 ⏳ + **P5 開放資料 ✅**
- **內容進度**：
...
- **§16 P3 全領域知識圖譜完成（2026-06-24 晚）**：
  - 總 nodes 4811 + 總 edges 1922（auto-detected stage-monotonic same-ordinal）
  - 12 個 graph（11 領域 + 總綱）全部 verify_graph.py 通過（6/6 self-test + 12/12 domain + _all-graph 一致性）
  - scripts/generate_graph.py（stdlib only，含 self-test，自動偵測邊 + 人工 edges.md 覆寫）
  - scripts/verify_graph.py（6 項檢查：node 數 / edge 起終點 / 無 cycle / stage 升序 / schema 合規 / 跨域一致）
  - 每個 domain：graph.json + graph.mermaid + README.md
  - 跨領域聚合：_all-graph.json + _schema.json
  - 本次實作：4811 nodes / 1922 edges，全部 stage 升序，0 cycle
- **§16-B P2-B 考卷/試題抓取收尾（2026-06-24 晚）**：
  - 總考卷 5655 個（米蘭老師 4968 + 大墩國小 ddes 524 + CEEC 163）
  - index_exams.py 改進：加 ddes 檔名代碼解析（C=國文/M=數學/S=社會/N=自然/H=健康/E=英文）+ 4.5 段「年級×科目交叉分佈」表
  - 國文/數學/社會/自然/英文/健康各年級分佈完整可查（INDEX.md §4.5）
...
| P3 | 全領域知識圖譜 — schema + generator + verify + 12 領域 graph.json + 跨域聚合全完成（4811 nodes / 1922 edges） | ✅ 2026-06-24 |
| P4 | 遊戲端整合介面 | ⏳（設計前必讀 `docs/motivation-research-collection-mechanics.md` + `docs/sdt-design-audit-framework.md` + `docs/p4-design-concept-pet-home.md`，歸 Claude A 管）|
| P5 | 開放資料釋出 — CITATION.cff + README 強化 + release-notes.md | ✅ 2026-06-25 |
...
## 16. P3 全領域知識圖譜章節（2026-06-24 完成）

### 16.1 產出統計

| 項目 | 數量 |
|------|------|
| 總 nodes | 4811（content + performance，跨 11 領域 + 總綱）|
| 總 edges | 1922（全部 auto-detected，0 manual）|
| domain graph 數 | 12（11 領域 + 總綱）|
| verify 結果 | 12/12 domain PASS + 6/6 self-test + 跨域一致性 PASS |

**各領域 nodes / edges**：

| Domain | Nodes | Edges | Stages |
|--------|------:|------:|--------|
| 健康與體育 | 571 | 355 | I-V |
| 國語文 | 331 | 233 | I-V |
| 國防 | 42 | 0 | V（only）|
| 數學 | 243 | 153 | I-V |
| 本土語文 | 961 | 316 | I-V（6 子檔合併）|
| 社會 | 595 | 250 | II-V |
| 科技 | 112 | 36 | IV-V |
| 綜合活動 | 252 | 124 | II-V |
| 總綱 | 0 | 0 | （無編碼）|
| 自然科學 | 876 | 130 | II-V |
| 英語文 | 711 | 246 | II-V（2 子檔合併）|
| 藝術 | 117 | 79 | II-V |

### 16.2 邊的自動偵測規則

同 domain + 同 category + 同 ordinal + 跨 stage 升序 → prerequisite 邊。

例如：`數學/N-1-1 → 數學/N-2-1 → 數學/N-3-1 → 數學/N-4-1 → 數學/N-5-1`（5 條鏈）。

**注意**：這只是 stage 升序的預設邊，**不一定語意正確**。真正的「學完 N-1-1 才能學 N-2-1」需人工驗證。

### 16.3 人工邊標註

每個領域可寫 `plans/<domain>/edges.md`（不存在就不讀），格式：
...
下次跑 generate_graph.py 自動合併進 edges（手動優先於 auto）。

### 16.4 已知限制 / TODO

- **Title / Description 自動抽取 v1 留空**：heuristic「從 raw_section_5 取編碼出現段落的前後 50 字」待 v2 實作
- **跨 domain 邊 v1 不做**：靠 `keywords` 提供弱搜尋能力（keywords v1 也空）
- **邊語意不保證正確**：auto-detected 是 stage 升序預設邊，非真實語意前備。需人工 review + 補 edges.md
- **Mermaid 視覺化限制**：> 50 nodes 圖會擠，到 https://mermaid.live 渲染較友善

### 16.5 下游使用（P4 遊戲端 / 開放資料）

- 遊戲端：讀 `_all-graph.json` → 從當前節點往前追 `prerequisite` 邊回答「要學什麼才能進入這關」
- 開放資料：每個 domain 的 `graph.json` 可獨立發布（MIT + CC BY 課綱衍生）
- `verify_graph.py` 是 CI 守門員：每次 push 前跑一次，fail 就擋住

---

## 17. 已知環境問題（給接手 Claude 看）

### 17.1 API 529 過載（2026-06-24 觀察到一次）

撞到點：`api.minimax.io` 推論閘道回 `529 The server cluster is currently under high load. Please retry after a short wait... (2064) (529).`

**這不是 Anthropic API 過載**，是 MiniMax 自己的推論集群暫時過載（MiniMax 內部 code 2064）。

**預防**：
1. 撞 529 → 等 30s → 重發同樣 prompt（Claude Code CLI 內建 retry 但只一次）
2. 長任務拆小 turn（單次 thinking 不要太重）
3. provider fallback 鏈暫不做（需要第二組 token）
4. **work checkpoint 早 commit** — 每完成一個 sub-task 就 `git add + commit + push`，session 死掉時只丟未提交的 sub-task（本 repo 在 2026-06-24 撞 529 時只丟了 3 檔 INDEX 重跑結果，不影響主程式）

**守護者補強**（未做）：tick-prompt.md 加 529 偵測 → 報 user。先標 TODO。

### 17.2 session 工作中斷（2026-06-24 觀察 pattern）
...
### 18.9 v8 ocr_and_relocate 完成（user pushback「做到完」觸發）

**user 第二次 pushback**：「不是說做到完為止嗎？沒做完為什麼停下來？」→ 不能停在「3000+ 未分科目」這關。

**scripts/ocr_and_relocate.py**（第一輪，幾分鐘跑完）：
1. **detect_subject_from_text 全文版**：不限 300 字，找 body 內全部 keyword（國文科/英文科/數學科/物理科/化學科/生物科/歷史科/地理科/公民科/自然科/社會科/健康與體育科）
2. **Group 推算**：用（學年+學期+年級+校名）group key，從同組其他 .md 的科目統計取最常見者推算（melances 一個學校同年級同學期通常同樣科目）
3. **OCR fallback**：body < 50 字跑 tesseract chi_tra
4. **Relocate**：shutil.move + 更新 frontmatter subject

**第一輪結果**：2470/2501 重新分類（53 OCR + 2447 Group 推算）

**scripts/force_ocr_remaining.py**（第二輪，~10 分鐘）：
- 對 31 個仍未分類跑強制 OCR（timeout 45s, 2 pages）
- 結果：3 個 relocate 成功（OCR 抓到數學/社會/英文）
- 28 個 OCR 也救不回（PDF 內容確實沒科目 keyword 或 OCR 失敗）— 已 OCR 寫回 .md

**最終：98.9% 重新分類成功**（2501 → 28 個真正無法分類）

| 主分類 | v7 數量 | v8 數量 | 改善 |
|--------|---------|---------|------|
| 國小-1 年級-數學 | 181 | **531** | +350 |
| 國小-2 年級-數學 | 224 | **649** | +425 |
| 國小-3 年級-國文 | 95 | **406** | +311 |
| 國小-4 年級-國文 | 100 | **426** | +326 |
| 國小-5 年級-國文 | 118 | **465** | +347 |
| 國小-6 年級-國文 | 89 | **337** | +248 |
| 國中-1 年級-國文 | 417 | **526** | +109 |
| 國中-2 年級-國文 | 427 | **521** | +94 |
| 國中-3 年級-國文 | 392 | **468** | +76 |
| 高中-1 年級-國文 | 192 | **204** | +12 |
| 「未分科目」 | 3000+ | **28** | 從 3000+ 縮到 28 |

**commit**: 5700652

### 18.10 下個 session 接著做（如有需要）

- **最後 28 個仍未分類**：手動查（ROC 114 國小 + 高中 ROC 109/110，可能是新學年或群組資料少）
...
## 19. P5 開放資料釋出（2026-06-25 完成 — v0.5.0）

### 19.1 user 觸發

user「**r繼續12年國教的進度**」觸發。§14.1 矩陣 P4 設計歸 A（Windows）不動；P5 沒人啟動過、且 repo 對外 metadata 才能讓他人引用 → 接 P5。

### 19.2 三檔釋出物

| 檔案 | 用途 |
|------|------|
| `CITATION.cff` | 學術引用 metadata（CFF 1.2.0），GitHub 自動顯示「Cite this repository」 |
| `README.md`（強化）| 從 P1 完成更新到 P0-P3 + P2-B 全完成 + 量化指標（4811/1922/5655/33/3237）+ 三類用法（遊戲端/老師/研究者）+ citation bibtex 範例 |
| `docs/release-notes.md` | 5 個 milestone（v0.1.0~v0.5.0）+ 各量化 + 已知限制 |

### 19.3 verify 結果（release 前）

```
verify_curriculum.py: 20/20 PASS, 0 fail, 0 warning
verify_graph.py:      12/12 PASS, 4811 nodes / 1922 edges
verify_exams_md.py:   9 fail（已知 §18.5 限制：8 ddes-未解析 BIG5 mojibake + 1 EXTRACT_FAIL unknown），
                      .gitignore 排除 working tree，不影響 git tracked state
```

### 19.4 scope 嚴格限制

- **未動 A 管範圍**：`docs/P4-*` / `docs/sdt-*` / `docs/motivation-*` / `docs/p4-design-concept-*` 仍歸 A
- **未動 P2-B 收尾**：§18.10 最後 28 個仍未分類（如有需要留給下個 session）
- **未動 P2 章節內文**：v3 --all（50 分鐘）需獨立 session，user 未指明要做

### 19.5 下個 session 接著做（如有需要）

- P4 遊戲端設計（**等 A 啟動**，B 不搶）
- §18.10 最後 28 個仍未分類（如有需要，手動查 ROC 114 國小 + 高中 ROC 109/110）
- P2 章節內文完整版（v3 --all，50 分鐘）
- P1.5 evidence gap 補研究（年齡層差異 / Hamari 結果方向 / 9 領域矩陣 / 其他案例長期效應 — 歸 A）
