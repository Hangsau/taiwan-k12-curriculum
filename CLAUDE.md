...
- **最後更新**：2026-06-23 晚
- **當前 Phase**：P1 結構化 + 驗證 ✅ + **P1.5 遊戲化動機研究 ✅**
- **內容進度**：
  - 20/20 領域純文字 markdown（P1 上半）
  - 20/20 領域結構化 JSON（P1 下半）
...
- **本次更新重點**：
  - **P1.5 遊戲化動機研究完成**：
    - `docs/motivation-research-collection-mechanics.md` 寫完（~18K 字 / 12 章 / 含 SDT 三大需求、CET controlling vs. informational、OIT 內化連續線、Deci 1971 經典實驗、Hamari 2015 方法學、Duolingo KDD 2020、DragonBox 雙機制 + 9 大領域 × 蒐集類型矩陣）
    - **結論**：蒐集機制不必然毀掉內在動機，關鍵在「框定方式」（controlling vs. informational）+ 是否滿足 SDT 三大需求
    - 對抗式驗證：25 claims 審核 → 8 confirmed / 15 refuted / 4 evidence gap（年齡層差異 / Hamari 結果方向 / 9 領域矩陣 / 其他案例長期效應）
  - **P1 結構化完成**：
...
| P1 | 抓 108 課綱（全部領域）+ 結構化 + 驗證 | ✅ 2026-06-23 |
| P1.5 | 遊戲化動機研究（前置研究給 P4） | ✅ 2026-06-23 |
| P2 | 抓部編本教材 | ⏳ |
| P3 | 全領域知識圖譜 | ⏳ |
| P4 | 遊戲端整合介面 | ⏳（設計前必讀 `docs/motivation-research-collection-mechanics.md`）|
| P5 | 開放資料釋出 | ⏳ |
...
*最後更新：2026-06-23 晚（scaffold + URL 修復 + P1 完成 20/20 + P1 結構化 + P1.5 遊戲化動機研究）*
